...
from vocoder import audiotorch
...
class VocoderDataset(Dataset):
    ...
    def __getitem__(self, index):  
        ...
        if hp.apply_preemphasis:
            wav = audiotorch.pre_emphasis(wav)
        wav = np.clip(wav, -1, 1)
        ...
        if hp.voc_mode == 'RAW':
            if hp.mu_law:
                quant = audiotorch.encode_mu_law(wav, mu=2 ** hp.bits)
            else:
                quant = audiotorch.float_2_label(wav, bits=hp.bits)
        elif hp.voc_mode == 'MOL':
            quant = audiotorch.float_2_label(wav, bits=16)
...
def collate_vocoder(batch):
    ...
    x = audiotorch.label_2_float(x.float(), bits)

    if hp.voc_mode == 'MOL' :
        y = audiotorch.label_2_float(y.float(), bits)
