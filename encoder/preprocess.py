...
from encoder import audiotorch
...
def _init_preprocess_dataset(dataset_name: str, datasets_root: str, out_dir: str) -> Tuple[Path, DatasetLog]:
    dataset_root = datasets_root.joinpath(dataset_name)
    if not dataset_root.exists():
        print("Couldn't find %s, skipping this dataset." % dataset_root)
        return None, None
...
def _preprocess_speaker(speaker_dir: Path, datasets_root: Path, out_dir: Path, skip_existing: bool):
    ...
    for extension in _AUDIO_EXTENSIONS:
        for in_fpath in speaker_dir.glob("**/*.%s" % extension):
            ...
            wav = audiotorch.preprocess_wav(in_fpath)
            if len(wav) == 0:
                continue

            # Create the mel spectrogram, discard those that are too short
            frames = audiotorch.wav_to_mel_spectrogram(wav)
